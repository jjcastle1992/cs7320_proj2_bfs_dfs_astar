...
def bfs2(start_board, goal_board):
    ...
    while (open_boards and (shortest_path == None)):
        ...
        print(f'**Current Board{bfs_level - 1}**')
        matrix_printer([current_board])
        print(f'Total Unique Child Boards at Level {bfs_level}: {num_children}')
        print('**All Current Child Boards to Explore**')
    ...
    print(f'Levels explored: {bfs_level}')
    print(f'Explored Node List Len: {len(explored_boards)}')
    print(f'Open Board List len{len(open_boards)}')
